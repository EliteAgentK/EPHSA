...
export function ParallaxSystem({ children }: { children: ReactNode }) {
  useEffect(() => {
    ...
    const heroLayers = Array.from(document.querySelectorAll<HTMLElement>("[data-hero-parallax]"));
    let heroFrame = 0;
    const updateHeroParallax = () => {
      heroFrame = 0;
      const scrollTop = Math.max(0, window.scrollY);
      heroLayers.forEach((layer) => {
        const strength = Number(layer.dataset.heroParallax ?? 4);
        const offset = Math.min(scrollTop * (strength / 100), mobileLayout ? 32 : 64);
        layer.style.transform = `translate3d(0, ${offset}px, 0)`;
      });
    };
    const requestHeroParallax = () => {
      if (!heroFrame) heroFrame = window.requestAnimationFrame(updateHeroParallax);
    };
    updateHeroParallax();
    ...
    return () => {
      ...
      heroLayers.forEach((layer) => layer.style.removeProperty("transform"));
      context.revert();
      if (lenis) {
